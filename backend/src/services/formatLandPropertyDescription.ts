...
export function formatLandPropertyDescription(data: LandPropertyData): string {
    ...
    return `
Поземлен имот с идентификатор ${data.identifier}, находящ се в ${data.address}, с площ ${data.area}, с трайно предназначение на територията: ${data.territoryPurpose}, с начин на трайно ползване: ${data.permanentUsage}, при стар идентификатор: ${data.previousIdentifier}, номер по предходен план: ${data.previousPlanNumber}, квартал: ${data.quarter}, парцел: ${data.parcel}, при съседи: ${neighbours}, ведно с построените в имота: ${buildings}.
    `.trim();
}