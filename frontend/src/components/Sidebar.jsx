...
const navItems = [
  ...
  { key: "navTemplates", icon: templatesIcon, active: false },
  { key: "navSettings", icon: settingsIcon, active: false },
];

function generateIcon(active) {
  return (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2 : 1.5} aria-hidden="true">
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
              item.active
                ? "bg-teal-50 text-teal-700 border-l-[3px] border-teal-600 rounded-l-none"
                : "text-slate-500 hover:bg-slate-50 cursor-not-allowed opacity-60"
            }`}
          >
