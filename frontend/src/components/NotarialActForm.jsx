...
function Input({ value, onChange, placeholder, type = "text" }) {
  return (
    <input
      ...
      className="w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition-colors"
    />
  );
}
...
function Select({ value, onChange, options }) {
  return (
    <select
      value={value}
      onChange={onChange}
      className="w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition-colors"
    >
      {options.map((opt) => (
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Section title={t(lang, "naSectionPayment")}>
        ...
        <Field label={t(lang, "naPriceCurrency")}>
          <Select
            value={form.priceCurrency}
            onChange={(e) => set("priceCurrency", e.target.value)}
            options={[
              { value: "евро", label: "EUR (евро)" },
              { value: "лева", label: "BGN (лева)" },
            ]}
          />
        </Field>
