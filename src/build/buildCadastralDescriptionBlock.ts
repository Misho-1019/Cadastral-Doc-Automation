...
export function buildCadastralDescriptionBlock({ pdfData, formattedPropertyAddress, formattedAttachedParts }: BuildCadastralDescriptionInput): string {
    ...
    return `- ${pdfData.property_purpose.toUpperCase()}, който съгласно Схема на самостоятелен обект в сграда № ${pdfData.scheme_number} г., издадена от АГКК, представлява самостоятелен обект в сграда с идентификатор ${propertyIdentifier}, с адрес: ${formattedPropertyAddress}, находящ се в сграда с идентификатор ${buildingIdentifier}, с предназначение: ${pdfData.building_purpose}, брой етажи: ${formattedBuildingFloors}, разположена в поземлен имот с идентификатор ${parcelIdentifier}, с предназначение на обекта: ${pdfData.property_purpose}, брой нива: ${formattedPropertyLevels}, с площ от ${pdfData.property_area} кв. м., прилежащи части: ${formattedAttachedParts}, при съседи: на същия етаж – ${pdfData.neighbors.same_floor}, под обекта – ${pdfData.neighbors.below}, над обекта – ${pdfData.neighbors.above}`;
}